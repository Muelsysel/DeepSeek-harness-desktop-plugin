...
2. **Electron shell** (`desktop/`): `main.cjs` (window, load-with-retry, single instance per port, external links → system browser, official DeepSeek whale icon on the window + taskbar; `--splash` mode opens a frameless progress window that polls a shared phase file and closes on `ready`), `splash.html` (animated whale progress bar, Chinese status), `preload.cjs`, `codex.css` (Codex skin via `--dsw-*` token overrides).
