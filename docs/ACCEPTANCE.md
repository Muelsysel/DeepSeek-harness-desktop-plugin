...
11. **Setup installer (NSIS, self-contained app)** — `setup\desktop-setup.nsi` + `scripts\make-setup.mjs` assemble the standalone-style app (bundled `backend/` + `electron/` runtime + shell with splash) into the payload; the compiler is bundled under gitignored `tools/`. Silent-install round-trip verified on this machine: `/S /D=<temp>` expands the app, creates the Desktop shortcut (Chinese name, whale icon), Start Menu entries and the `dev.dsh.desktop` uninstall entry; `/S` uninstall removes the tree, both shortcut sets and the registry entry. The installed app boots its own backend into a private `%APPDATA%` profile (launch verified: splash progress → `dsh web: http://127.0.0.1:<port>` → window handoff), with no Node/pnpm/dsh required.
...
13. **Startup splash (plugin)** — the launcher spawns `desktop\main.cjs --splash` immediately and feeds phase tokens via `%TEMP%\dsh-desktop-splash.status` (`download` when npx will fetch dsh, `boot` for a global dsh, `loading` written by the shell, `ready` on the main window's `ready-to-show`). Verified live: the splash process stays up through `boot`, closes within a polling tick of `ready`, and deletes the status file; the launcher e2e shows the final token is `ready`.
14. **pnpm auto-install** — `install-profile.mjs` probes pnpm through a shell on Windows (the `.cmd`-shim constraint), and auto-installs via corepack then `npm install -g` when missing; a missing profile is skeleton-created before registration. Verified: fresh-profile registration succeeds end-to-end; the import side-effect guard (module must not run `main()` on import) is covered by the unit suite.
15. **No-space install path** — the NSIS installer installs to `%LOCALAPPDATA%\Programs\DeepSeek-Harness-Desktop` (pnpm rejects `link:` specs containing spaces); silent install/uninstall round-trip re-verified at the new path.
...
- **Script-free shortcuts**: a root "`DeepSeek Harness 桌面版.lnk`" (send-to-desktop, no scripts needed) ships in the repo and the zip root; the NSIS installer now also drops an explicit `DeepSeek Harness Desktop.lnk` launcher inside the install dir. A portable (relative-path) `.lnk` is not possible in the Windows shortcut format — the WorkingDirectory must be absolute — so the root `.lnk` targets the build location and `create-shortcut.cmd` remains the documented one-time fix for other extraction paths.
- **README**: content optimized and an English version added (`README.en.md`, linked both ways with `README.md`).
